...
We will attempt to design a technique that follows a more principled approach that attempts to construct smooth goal-connecting paths in output space rather than relying on a random search algorithm on the error-weight surface.
...
\end{equation}
Since $\theta$ is the angle of the tangent at $C'$, we obtain from \ref{eq:clothoid_tangent_angle} that $\theta=\frac{\pi}{2} \left(t_C\right)^2$.
...
By extension, we can then given $\vec{q}$ in output space in terms of $\vec{c}$ and $\vec{b}$ as $\vec{q} = 2\vec{c} - \vec{b}$.
...
This is the two-dimensional plane\footnote{This plane is a subspace of the output space, as the latter may well be of higher dimensionality.} given by our three known points $A,B,C$ or equivalently by the basis vectors $\vec{b}$ and $\vec{c}$ which we will denoted by the matrix
$\vec{A} = \begin{bmatrix}
    \vec{b} & \vec{c}
\end{bmatrix}$.
...
The na\"{i}ve algorithm from \ref{sec:subgoal_computation} proved to be a bottleneck in the pre-computation stage, so a more efficient version of that algorithm was devised.
...
This clothoid will dictate a subgoal as per \ref{sec:subgoal_computation} which we will attempt to achieve by extending or shrinking the current weight configuration.
...
Let us formalise these steps below.
Note that in implementing the algorithm, instead of actually drawing clothoids that pass through a given triple of points, we will simply look up the clothoid parameters using the angles $\langle \gamma_1, \gamma_2 \rangle$ formed by these points.
\begin{enumerate}
    \item Record the starting point $\vec{s}$ in output space.
    \item Make an initial move in weight space producing the sample point $\vec{p}$ in output space.
    \item Draw an initial clothoid using the triple of initially known points, $\langle \vec{s}, \vec{p}, \vec{g} \rangle$ where $\vec{g}=\vec{y}$ is the goal.
        Look up the associated value of $\theta$ and record it as $\theta_\text{old}$.
    \item Employ a sampling technique such as in \ref{sec:greedy_probing}
         \todo
\end{enumerate}
