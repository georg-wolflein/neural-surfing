    ...
    \begin{subfigure}{.45\textwidth}
        \begin{tikzpicture}[
            ...
            \begin{axis}[
                ...
                \node[circle,fill,inner sep=1.5pt,pin=80:{$\vec{\hat{y}}_0$}] at (0, 0) {};
                \node[circle,fill,inner sep=1.5pt,pin=100:{$\vec{\hat{y}}_S$}] at (1, 0) {};
                \node[circle,fill,inner sep=1.5pt,pin={[align=center]90:{unrealisable\\point}}] at (.5, 0) {};
            \end{axis}
        \end{tikzpicture}
        \caption{Output space.}
    \end{subfigure}
    \caption{Ideal goal-connecting path in weight and ouput spaces. It suffices to show the first and third samples of the output space as the second stays constant. Note also that $\vec{\hat{y}}_S=\vec{y}$ because the target is in fact realisable. The point labelled `unrealisable' is the example of an unrealisable point referred to in \ref{sec:stripe_ideal_goal_connecting_path}.}
...
\begin{figure}
    ...
    \begin{subfigure}{.45\textwidth}
        \begin{tikzpicture}[
            declare function={
                hminus(\x,\v,\w)= (-\v*\x - sqrt(-ln(0.5))) / \w;
                hplus(\x,\v,\w) = (-\v*\x + sqrt(-ln(0.5))) / \w;
                h(\x,\v,\w)     = -\v*\x / \w;
            }
        ]
            \begin{axis}[
                ...
                \addplot[black, no marks] {hminus(x,1,-1-.5*sqrt(ln(2)))};
                \addplot[black, no marks] {hplus(x,1,-1-.5*sqrt(ln(2)))};
                \addplot[black, dashed] {h(x,1,-1-.5*sqrt(ln(2)))};
                \fill (2, 2) circle (2pt) node[above] {$\vec{x}_1$};
                \fill (0, 2) circle (2pt) node[above] {$\vec{x}_2$};
                \fill (2, 0) circle (2pt) node[above] {$\vec{x}_3$};
            \end{axis}
        \end{tikzpicture}
        \caption{$w_1=1$ and $w_2= - \frac{1}{2}\sqrt{\ln{2}}-1$}
    \end{subfigure}
    \hspace*{\fill}
    \begin{subfigure}{.45\textwidth}
        \begin{tikzpicture}[
        ...
        \caption{$w_1=1$ and $w_2= \frac{1}{2}\sqrt{\ln{2}}-1$}
    \end{subfigure}
    \caption{Input space with hyperplanes and zero-excitation line (dashed) of two of the four possible configurations where a hyperplane of $0.5$ excitation passes through $\vec{x}_1$. The other two configurations can be obtained by reflecting the plots about the $x_1=x_2$ line.}
    \label{fig:stripe_unrealisable_example_hyperplanes}
\end{figure}
To see this, consider for example the point
$\begin{bmatrix}
    0.5 & 0 & 0
\end{bmatrix}\tran$. 
...
\begin{wrapfigure}{r}{.6\textwidth}
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            \addplot[mark=none,blue,dashed]  table {data/stripe_problem_weights_history_4.dat};
            \addplot[mark=none,red,dashed]   table {data/stripe_problem_weights_history_5.dat};
            \addplot[mark=none,black,dashed] table {data/stripe_problem_weights_history_6.dat};
            \node[circle,fill,inner sep=1.5pt,pin=-90:{$\vec{w}_0$}] at (1, 1) {};
            \node[circle,fill,inner sep=1.5pt,pin=90:{$\vec{w}_S$}] at (1, -1) {};
            \legend{Trial 1,Trial 2,Trial 3,Trial 4,Trial 5,Trial 6}
        \end{axis}
    \end{tikzpicture}
    \caption{Some gradient descent trajectories in weight space.}
\end{wrapfigure}
...
\todo: refer to \ref{sec:neural_training_issues}, specifically \ref{fig:gradient_descent_narrow_valley} due to valley
