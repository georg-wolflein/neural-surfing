...
\todo: Explain significance if target is unrealisable (XOR example demonstrates that)
...
\begin{example}
    ...
    Like in Example \ref{ex:sln_2_in_1_out_sigmoid_hyperplane}, let us consider once more a single-layered MLP $M$ with two inputs, depicted in \ref{fig:sln_2_in_1_out}, where $w_1=w_2=1$ and $b=0$. 
...
\begin{proof}
    ...
    $\vec{J}_L = \vec{0}$.
    It is trivial to see that $\vec{w}_1$, $\vec{w}_2$, and $\vec{w}_3$ are solutions to this equation.
\end{proof}
...
\begin{proof}
    ...
    Since $w_1=w_2$ we get the solutions $\vec{w}_4$ and $\vec{w}_5$.
\end{proof}
...
\begin{lemma}
    ...
    The local minima on the error-weight surface given by $L$ are $\vec{w}_1$, $\vec{w}_2$, $\vec{w}_3$.
\end{lemma}
\begin{proof}
    ...
    Calculating the Hessian for $\vec{w}_4$ numerically, we obtain
    \begin{equation*}
    ...
    The Hessian obtained at $\vec{w}_5$ is equal to the Hessian at $\vec{w}_4$. 
    It follows that this point is a local minimum as well.
    In summary, we have shown that $\vec{w}_1$, $\vec{w}_2$, $\vec{w}_4$, $\vec{w}_5$ are local minima, but not $\vec{w}_3$.
\end{proof}
...
\ref{fig:stripe_critical_points} shows the critical points which allows the hypothesis that if we have identified all local minima, then $\vec{w}_1$ and $\vec{w}_2$ should be the global minima.
While we were not able to prove that the five critical points we identified are the only ones, we \textit{can} prove that $\vec{w}_1$ and $\vec{w}_2$ are in fact global minima because their loss values minimal.
\begin{figure}
    \centering
    \begin{tikzpicture}[
        declare function={
        ...
        \begin{axis}[
            ...
            \node[label={360:{$\vec{w}_1$}},circle,fill,inner sep=2pt] at (-1, 1, 0) {};
            \node[label={360:{$\vec{w}_2$}},circle,fill,inner sep=2pt] at (1, -1, 0) {};
            \node[label={360:{$\vec{w}_3$}},circle,fill,inner sep=2pt] at (0, 0, 2) {};
            \node[label={360:{$\vec{w}_4$}},circle,fill,inner sep=2pt] at (1, 1, 0.999) {};
            \node[label={360:{$\vec{w}_5$}},circle,fill,inner sep=2pt] at (-1, -1, 0.999) {};
        \end{axis}
    \end{tikzpicture}
...
\begin{theorem}
    ...
    The points $\vec{w}_1=\begin{bmatrix}
        -1 & 1
    \end{bmatrix}\tran$ and $\vec{w}_2=\begin{bmatrix}
        1 & -1
...
\begin{proof}
    ...
    Let us look at the value of the loss function at $\vec{w}_1$,
    \begin{align*}
        L(\vec{w}_1)
        &=\left(e^{-4(w_1 + w_2)^2} - 1\right)^2
        + \left(e^{-4w_2^2} - e^{-4}\right)^2
    ...
    \end{align*}
    Similarly, we obtain $L(\vec{w}_2)=0$. It follows that both $\vec{w}_1$ and $\vec{w}_2$ are global minima.
\end{proof}
\begin{corollary}
    \label{col:stripe_suboptimal_local_minima}
    The points $\vec{w}_4$ and $\vec{w}_5$ are suboptimal local minima.
\end{corollary}
\begin{proof}
    We have already shown in \ref{lmm:stripe_local_minima} that $\vec{w}_4$ and $\vec{w}_5$ are local minima. 
    Let us look at the loss values at $\vec{w}_4$.
    \begin{align*}
        L(\vec{w}_4)
        ...
        + \left(e^{-4\eta^2} - e^{-4}\right)^2
    \end{align*}
    Since $\eta > 0$, we conclude that $L(\vec{w}_4)>0$. 
    In \ref{thm:stripe_global_minima} we have shown that the minimum of $L(\vec{w})$ is zero.
    Thus, by \ref{def:global_minimum}, it follows that $\vec{w}_4$ is \textit{not} a global minimum, so it is a suboptimal local minimum.
    By extension, since $L(\vec{w})=L(-\vec{w})$ and $\vec{w}_4=-\vec{w}_5$, it must be the case that $\vec{w}_5$ represents a suboptimal local minimum as well.
\end{proof}
...
In fact, if $w_1$ and $w_2$ are either both positive or both negative, gradient descent will converge to a suboptimal local minimum, whereas if $w_1$ and $w_2$ have different sign, gradient descent training will converge to a global minimum.
This can be seen in the heat map of \ref{fig:stripe_critical_points}.

To see this, consider for example $\vec{w}_4$. 
For $w_1>\eta$ and $w_2>\eta$, it can be shown that $\vec{J}_L>\vec{0}$.
More specifically, it can be shown that when $w_1>w_2>\eta$, the angle $\theta$ of the gradient of the loss function (when using $w_1$ as the $x$-axis and $w_2$ as the $y$-axis) is in the interval $\left(0, \frac{\pi}{2}\right)$.
As a consequence, the angle of the negative gradient will point in the rough direction of $\vec{w}_4$.
\todo: diagram showing directions of gradients


\subsection{Ideal goal line}
\todo: graph of ideal goal line in output space + analysis of unrealisable region

\subsection{Backpropagation applied to the stripe problem}
