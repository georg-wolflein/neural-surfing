...
\begin{proof}
    ...
    By Definition \ref{def:mlp}, this is equivalent to SLN $S$ with $m$ inputs and one output as shown in \ref{fig:sln_m_in_1_out}. 
    \begin{figure}
        \begin{center}
            \begin{tikzpicture}
                [
...
\begin{proof}
    ...
    By Definition \ref{def:unrealisable_region}, $\mathcal{U} \subset \mathcal{O}_A$, so it remains to prove that every unrealisable region $\mathcal{U} \neq \mathcal{O}_A$.
...
\begin{definition}[Goal-connecting path]
    ...
    The goal-connecting path is \textit{realizable} if and only if no subgoal is a strongly unrealisable point (Definition \ref{def:unrealisable_point}).
...
\end{proof}
For the remainder of this project, we will assume that these three solutions are the only ones. 
Analysing the graph from \ref{fig:stripe_error_surface_gaussian} does suggest so.

\begin{lemma}
    The local minima on the error-weight surface given by $L$ are $\vec{w}_1$ and $\vec{w}_2$.
\end{lemma}
\begin{proof}
    ...
    In summary, we have shown that $\vec{w}_1$ and $\vec{w}_2$ are local minima, but not $\vec{w}_3$.
\end{proof}



\subsection{Global minima}
\todo: prove that $\vec{w}_1$ and $\vec{w}_2$ are global minima (plug in loss function and observe $L=0$)

\subsection{Behaviour as the weights approach infinity}
\todo: show that there is a ``local minimum'' when both weights tend to infinity (or negative infinity)
...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{A secenario in two-dimensional output space. Note that the ideal goal line need not beparallel to the $y_1$ axis.}
...
Once we have determined $t_\mathcal{B}$ and $t_\mathcal{C}$ and thus also know the coordinates of $\mathcal{B}$ and $\mathcal{C}$, we can find $\beta$ as the angle that $\mathcal{C}$ makes with the $x$-axis, giving
\begin{equation}
...
\begin{figure}
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            \addplot[mark=none] table {data/clothoid_for_points_1.dat};
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{A scenario in two-dimensional output space. The sizes of the angles $\alpha,\beta,\gamma_1,\gamma_2$ are identical to \ref{fig:clothoid_motivation_scenario,fig:clothoid_segment}. The dotted lines are the tangents of the clothoid at $A$ and $C$.}
