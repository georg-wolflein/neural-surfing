...
Backpropagation (BP) via gradient descent is an iterative algorithm for training neural networks that, provided a suitable learning rate $\alpha$, is guaranteed to converge to a \textit{local minimum}\footnote{We will provide a formal discussion of this concept in Section \ref{sec:local_minimum_problem}.}.
...
In the exhaustive case, the samples are given by
\begin{equation*}
    \left\{
        \vec{p} + r
        \frac{
...
On the other hand, the random sampling technique will generate $n$ random samples in weight space with the condition that for every sample $\vec{p}_i$, it must be true that
$\norm{\vec{p} - \vec{p}_i} = r$.
...
\begin{example}
    ...
    \begin{figure}
        ...
        \begin{tikzpicture}[
            ...
            \end{axis}
        \end{tikzpicture}
        \caption{Plot of the input space of the MLP from Figure \ref{fig:sln_2_in_1_out} with Gaussian RBF activation where $w_1=w_2=1$, $b=0$, and some sample data.}
    ...
    \begin{tikzpicture}[
        ...
        \begin{axis}[
            ...
            \addplot3 [
                ...
                domain=-3:3,
                y domain=-3:3
            ] {(rbf(2*x))^2 + (rbf(2*y))^2 + (1 - rbf(2*x + 2*y))^2};
