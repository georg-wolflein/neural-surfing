...
\begin{definition}[Single-layer network]
    ...
    \begin{equation}
        ...
        \vec{S}(\vec{x}; \vec{W}, \vec{b}, \vec{g}) = \vec{g}\left( \vec{W}\tran \vec{x} + \vec{b} \right)
    \end{equation}
    ...
    \begin{equation}
        ...
        \end{bmatrix}
    \end{equation}
    captures all weights, the vector
    \begin{equation}
        \label{eq:sln_bias}
        \vec{b} = \begin{bmatrix}
            b_1 & b_2 & \cdots & b_n
        \end{bmatrix}\tran
    \end{equation}
    represents the biases, and $\vec{g}$ is the vector-valued activation function.
\end{definition}
...
\begin{definition}[Multi-layer perceptron]
    ...
    \begin{equation}
        M(\vec{x})
            ...
            \right)
    \end{equation}
    where $\vec{f}_l(\vec{x}) = \vec{S}(\vec{x}; \vec{W}_l, \vec{b}_l, \vec{g}_l)$ for $l = 1, \dots, L-1$. 
    The outermost function $f_L$ represents a SLN with only one output unit and is hence the scalar-valued function $f_L(\vec{x}) = S(\vec{x}; \vec{W}_L, \vec{b}_L, \vec{g}_L)$.
...
\begin{definition}[Weight space]
    ...
    The trainable parameters are its weights and biases.
...
It follows that $\mathcal{W}$ and $\mathcal{O}$ are usually spaces of different dimensionalities. 

We will now examine whether there exists a linear mapping between these two spaces. 
Note that linear mappings can exist between spaces of different dimensionalities \cite{rudin2006}.

\begin{theorem}
    \label{thm:sln_one_output}
    For a SLN with one output unit, the function $h: \mathcal{W} \rightarrow \mathcal{O}$ mapping the weights to outputs is not a linear mapping.
\end{theorem}
\begin{proof}
...
\end{corollary}
\begin{proof}
    Consider a MLP $M$ with $L$ layers.
